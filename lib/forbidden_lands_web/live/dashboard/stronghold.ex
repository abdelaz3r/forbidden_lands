defmodule ForbiddenLandsWeb.Live.Dashboard.Stronghold do
  ...
  def stronghold(assigns) do
    ...
    <section
      :if={@stronghold}
      class={[
        "flex-none border-t border-stone-900 shadow-2xl shadow-black/50 bg-gradient-to-l from-stone-800",
        "to-stone-900 transition-all duration-500 relative",
    ...
    >
      ...
      <div class="p-4">
        ...
        <div class="flex gap-4 mb-4">
          <div class="flex grow gap-2 border border-stone-800 p-4 bg-stone-900/60 justify-around">
          ...
          <div class="border border-stone-800 py-2 px-4 bg-stone-900/60 justify-around flex gap-2 items-center flex-none">
            <.icon
              name={:message_circle}
        ...
        <div class="grid grid-cols-2 gap-1 tabular-nums">
          <div :for={type <- Stronghold.resource_fields()} :if={type != :coins} class="flex gap-1 items-baseline">
            <span class="inline-flex font-bold items-baseline">
              <span class="text-stone-100/10"><%= leading_zeros(Map.get(@stronghold, type)) %></span>
              <%= Enum.min([9999, Map.get(@stronghold, type)]) %>
            </span>
            <span class="text-stone-100/60 text-sm truncate">
  ...
  defp coins_class(:silver), do: "bg-stone-200 border-stone-100 outline-stone-100/10 text-stone-700"
